...
public interface ArticleScheduleRepository extends JpaRepository<ArticleSchedule, Long> {
    ...
    List<ArticleSchedule> findByArticleJpaEntityAndStatusOrderBySortOrderAsc(
        ArticleJpaEntity articleJpaEntity, ArticleScheduleStatus status);

    // 일정 상세 리스트 - 삭제를 위한 비교 데이터
    List<ArticleSchedule> findByArticleJpaEntityAndStatusOrderByIdDesc(ArticleJpaEntity articleJpaEntity, ArticleScheduleStatus status);
}
