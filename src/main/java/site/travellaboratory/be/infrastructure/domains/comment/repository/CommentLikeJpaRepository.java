...
public interface CommentLikeJpaRepository extends JpaRepository<CommentLikeJpaEntity, Long> {
    ...
    List<CommentLikeJpaEntity> findAllByUserIdAndCommentIdInAndStatusFetchJoinComment(@Param("userId") Long userId, @Param("commentIds") Collection<Long> commentIds, @Param("status") UserLikeCommentStatus status);
}
